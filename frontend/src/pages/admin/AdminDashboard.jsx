...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ padding: 32 }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 6 }}>Tổng quan Dashboard</h1>
      <p style={{ color: "#6b7280", marginBottom: 24 }}>Chào mừng bạn! Đây là tình hình hôm nay.</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 2fr) minmax(0, 1fr)", gap: 16, marginTop: 24 }}>
        <div style={{ background: "#fff", borderRadius: 12, padding: 20, border: "1px solid #f3f4f6" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <h3 style={{ margin: 0, fontSize: 16, fontWeight: 600 }}>Xu hướng chấm công</h3>
        ...
        <div style={{ background: "#fff", borderRadius: 12, padding: 20, border: "1px solid #f3f4f6" }}>
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 600, marginBottom: 12 }}>Việc cần xử lý</h3>
          {pendingActions.length === 0 ? (
            <p style={{ color: "#6b7280" }}>Không có việc cần xử lý.</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {pendingActions.map((item, index) => (
                <div key={`${item.type}-${index}`} style={{ padding: 12, border: "1px solid #e5e7eb", borderRadius: 10 }}>
                  <div style={{ fontSize: 12, color: "#6b7280" }}>{item.type}</div>
                  <div style={{ fontWeight: 600 }}>{item.title}</div>
                  <div style={{ fontSize: 12, color: "#9ca3af" }}>{item.date ? new Date(item.date).toLocaleDateString("vi-VN") : ""}</div>
                </div>
              ))}
            </div>
          )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1fr)", gap: 16, marginTop: 24 }}>
        <div style={{ background: "#fff", borderRadius: 12, padding: 20, border: "1px solid #f3f4f6" }}>
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 600, marginBottom: 12 }}>Nhân viên theo phòng ban</h3>
          {departmentStats.length === 0 ? (
            <p style={{ color: "#6b7280" }}>Chưa có dữ liệu phòng ban.</p>
          ) : (
        ...
        <div style={{ background: "#fff", borderRadius: 12, padding: 20, border: "1px solid #f3f4f6" }}>
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 600, marginBottom: 12 }}>Thống kê nhanh</h3>
